...
rect.hclust(hclust(dist(y), method = "complete"), k = 3)
#####################################################################
##The inversion
x1 <- c(0, 0)
x2 <- c(1, 0)
...
cutree(hc.single, 2)
cutree(hc.single, 4)
#to compare between data scaled and not scaled
xsc <- scale(x)
par(mfrow = c(1, 2))
plot(hclust(dist(x), method = "complete"), 
     main = "Hierarchical Clustering without Scaled Features", 
     xlab = "", sub = "", cex = .9)
plot(hclust(dist(xsc), method = "complete"), 
     main = "Hierarchical Clustering with Scaled Features", 
     xlab = "", sub = "", cex = .9)
par(mfrow = c(1, 1))
#to cluster with correlation coefficients
