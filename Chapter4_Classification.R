...
glm.fit3 <- glm(default ~ balance + income + student, 
...
glm.fit4 <- glm(default ~ balance + student, data = Default, 
...
interval_len <- 2700/N 
for(i in 1:N){
  student_default <- Default[which(Default$balance <= (interval_len * i-1) & 
                                   Default$balance >= ((i - 1) * interval_len) & 
                                   Default$student == "Yes"), "default"]
  non_student_default <- Default[which(Default$balance <= (interval_len * i-1) &
                                       Default$balance >= (i - 1) * interval_len &
                                       Default$student == "No"), "default"]
  student_default_rate[i] <- sum(student_default == "Yes") / 
                             length(student_default)
...
non_student_average_default_rate <-  
  length(Default[Default$student == "No" & Default$default == "Yes", 1])/
...
legend("topleft", inset = .05, 
       title = "default Comparison",c("Student","Non-Student"), 
       lty = c(1,1), col = c("orange","blue"))
predict.glm(glm.fit3, 
            newdata = data.frame(student = "Yes", balance = 1500, income = 40000),
            type = "response")
predict.glm(glm.fit3, 
            newdata = data.frame(student = "No", balance = 1500, income = 40000),
            type = "response")
#####################################################################
...
#to draw the picture between thresholds and different error rates
...
for(i in 1:length(threshold)){
  predict_group_new <- 
  as.numeric(predict(default_forecast)$posterior[, "Yes"] > threshold[i])
  ...
  if(sum(predict_group_new) == 0){
        classification_error_rate[i] =  
        sum(original_group2)/length(original_group2)
        ...
        classification_error_rate[i] =  
        1 - sum(original_group2) / length(original_group2)
        classification_error_rate_of_default[i] = 0
        classification_error_rate_of_non_default[i] = 1
  }else{
        classification_error_rate[i] <- 
        (table_new[1, 2] + table_new[2, 1]) / dim(Default)[1] 
        classification_error_rate_of_default[i] <- 
        table_new[2, 1] / sum(table_new[2, ])  
        classification_error_rate_of_non_default[i] <- 
        table_new[1, 2] / sum(table_new[1, ])
  }
}
plot(classification_error_rate ~ threshold, 
     xlab = "Threshold", ylab = "Error Rate",
     type = "l", col = "orange", lwd = 3, ylim = c(-0.1, 1.1))
lines(classification_error_rate_of_default ~ threshold, 
      col = "blue", lty = 2, lwd = 3)
lines(classification_error_rate_of_non_default~ threshold, 
      col = "red", lty = 3,lwd = 3)
legend("topleft", inset = .05, title = "Comparison",
       c("All","Default","NoDefault"), 
       lty = c(1,2,3), col = c("orange","blue","red"), lwd = 3)
#ROC Curve
...
for(k in 1:N){
      ...
      simulated_data <- data.frame(x = c(group0[, 1], group1[, 1]), 
                                   y = c(group0[, 2], group1[, 2]),
                                   group = group)
      training_data <- simulated_data[c(1:20, 1021:1040),]
      test_data <- simulated_data[-c(1:20, 1021:1040), -3]
      ##########################################################################
      #lda model################################################################
      ##########################################################################
      lda_fit <- lda(group ~ ., data = training_data)
      lda_prediction <-predict(lda_fit, newdata = test_data)
      error_rate[k, 1] <- 
      ...
      qda_prediction <- predict(qda_fit, newdata = test_data)
      error_rate[k + N, 1] <- 
